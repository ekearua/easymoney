...
The ladder is enforced in the store (`AdvanceKYCTier`/`AdvanceKYCTierTo`/`DowngradeKYCTier`): advances must be adjacent with the matching evidence, downgrades can drop to any lower tier, and a sanctions/PEP screening decision of `strong`/`blocked` halts advancement until cleared (`clear`/`manually_cleared`). Every transition, screening, and manual review is written to the audit log (`kyc.tier_advanced`, `kyc.tier_downgraded`, `kyc.review_approved`, `kyc.review_rejected`, `kyc.screening_recorded`).

Sanctions/PEP screening runs through the `SanctionsScreener` port (`internal/ports`). `SCREENING_PROVIDER=simulated` ships a deterministic provider (a legal name containing a sanctions token like "Sanctioned"/"Drug Lord" returns `strong`; a PEP token like "Senator"/"Minister" returns `possible`; everything else is `clear`) — a live OFAC/vendor integration plugs in behind the same interface. Screening runs at onboarding before the L2 promotion, and a background worker re-screens profiles every `KYC_RESCREEN_PERIOD` (default 90 days, `go run ./cmd/demo rescreen` for a manual run); a `strong`/`blocked` rescreen downgrades the profile to L1 and opens a review case.

In the demo flow, an individual profile upgrade confirms the channel and submits an identity, is sanctions/PEP screened, and — if clear — promotes the user to L2 (the level that unlocks thrift creation). A `strong`/`blocked` screening parks the profile at L1 and opens a manual review case instead. The user can then submit an 11-digit NIN or BVN, verified through the `IdentityVerifier` port (`internal/ports`), which records the verification and advances the profile to **L3** — `IDENTITY_PROVIDER=simulated` ships a deterministic local provider (a NIN starting with `1` or a BVN starting with `2` verifies; `8…` mismatches; `9…` is not found), and a live vendor (Smile ID / Youverify / Dojah / Prembly) plugs in behind the same interface. Compliance teams manage the ladder and the manual review queue at `/admin/kyc` (admin/compliance roles): tier distribution, screening outcomes, and approve/reject actions on `pending` cases.
...
### Cross-channel identity carry-over

KYC tier is **global** to the customer, not per channel: the surviving primary account's `account_level`, `verification_level`, and KYC profile follow the customer across WhatsApp, Telegram, Instagram, and TikTok. A customer whose global identity already cleared the approved/money-out tier (individual account label with KYC ≥ L2) is therefore never re-hijacked into per-channel onboarding on a fresh channel. When such a user first messages from an un-stamped channel, `Handle` routes to `handleNewChannelForApprovedUser` (`internal/service/conversation_onboarding.go`): they get a one-time, non-blocking intro ("It's the same you — your <channel> is now linked to your Xego account") followed by the menu, and the channel is durably stamped via `MarkChannelOnboarded` (a KYC-neutral `UPDATE` that touches only that channel's `*_confirmed_at` via `COALESCE`, never `verification_level`, `account_level`, or KYC tier). The stamp closes the gate so the intro fires exactly once. Users who have NOT reached an approved global tier still go through per-channel account confirmation exactly as before. Regression coverage: `internal/service/conversation_channel_carryover_test.go` (approved user gets intro + menu, no confirmation hijack; unapproved user still onboards) and `internal/store/store_mark_channel_integration_test.go` (stamp is idempotent, KYC-neutral, and covers WhatsApp/Telegram/Instagram/TikTok).
...
Cards collect through the **Hosted Fields** SDK (`INTERSWITCH_CHECKOUT_RENDER=hosted_fields`, the default), which renders Interswitch's secured fields in inline iframes so card data never touches the platform; set `INTERSWITCH_CHECKOUT_RENDER=legacy` for the Web Checkout redirect form, which posts to Interswitch only when the customer deliberately taps **Continue to Interswitch** (no page-load auto-submit). Both pages offer an inline change-payment-method link when the payment came from a browser web flow.
...
- Analytics: `/admin/analytics` dashboard with daily revenue, merchant settlement summaries, transaction volume, and payout history. Filterable by date range with CSV export per report section.
