...
func (s *Store) RunReconciliation(ctx context.Context, runType, createdBy string) (ReconciliationRun, []ReconciliationItem, error) {
	...
	// Leg 4 (S1/W1): completed payouts must have a money-out posting; every
	// such posting must belong to a completed payout. Money-out is the debit
	// with source_type 'payout' — on the settlement payable (legacy) or on the
	// merchant's business wallet (3101_business_wallet:<id>, W1).
	...
	rows, err = tx.Query(ctx, `
		...
		FROM ledger_entries
		WHERE entry_type='debit' AND source_type='payout' AND reversal_of IS NULL
		  AND (account = $1 OR account LIKE $2)
		GROUP BY source_id`, LedgerAccountSettlementPayable, LedgerAccountBusinessWallet+":%")
	if err != nil {
		return ReconciliationRun{}, nil, err
	}
	for rows.Next() {
